...
func (b *backend) CreateCalendar(ctx context.Context, calendar *caldav.Calendar) error {
	return fmt.Errorf("caldav/CreateCalendar: creating calendars is not supported")
}
...
func (b *backend) PutCalendarObject(ctx context.Context, path string, calendar *ical.Calendar, opts *caldav.PutCalendarObjectOptions) (*caldav.CalendarObject, error) {
	...
	newPath := homeSetPath + calId + formatCalendarObjectPath(newEvent.ID)
	
	// Return the updated calendar object
	co, err := b.GetCalendarObject(ctx, newPath, nil)
	if err != nil {
		return nil, fmt.Errorf("caldav/PutCalendarObject: error getting updated calendar object: (%w)", err)
	}
